...
import yaml
from data_utils_SSL_eval import genSpoof_list,Dataset_ASVspoof2019_train,Dataset_ASVspoof2021_eval,Dataset_in_the_wild_eval,Dataset_OOD_eval
...
if __name__ == '__main__':
    ...
    print("[DEBUG] Model initialized successfully.", flush=True)
    nb_params = sum([param.view(-1).size()[0] for param in model.parameters()])
    
    model = nn.DataParallel(model)
    print("Using devices:", list(model.device_ids), flush=True)
    ...
    if args.eval:
        file_eval = genSpoof_list(dir_meta ='/export/fs05/aurosweta/Big_Datasets/ASVspoof_2019/LA/LA/ASVspoof2019_LA_cm_protocols/ASVspoof2019.LA.cm.eval.trl.txt',is_train=False,is_eval=True)
        print('no. of eval trials',len(file_eval),flush=True)
        eval_set=Dataset_ASVspoof2021_eval(list_IDs = file_eval,base_dir ='/export/fs05/aurosweta/Big_Datasets/ASVspoof_2019/LA/LA/ASVspoof2019_LA_eval/flac/')
        produce_evaluation_file(eval_set, model, device, args.eval_output)
        sys.exit(0)
    ...
    # d_label_trn,file_train = genSpoof_list( dir_meta = '/export/fs05/aurosweta/Big_Datasets/ASVspoof_2024/Protocols/ASVspoof5.train.tsv',is_train=True,is_eval=False)
    
    # print('no. of training trials',len(file_train),flush=True)
    
    # train_set=Dataset_ASVspoof2019_train(args,list_IDs = file_train,labels = d_label_trn,base_dir = '/export/fs05/aurosweta/Big_Datasets/ASVspoof_2024/train_set/',algo=args.algo)
    ...
    # d_label_dev,file_dev = genSpoof_list( dir_meta =  '/export/fs05/aurosweta/Big_Datasets/ASVspoof_2024/Protocols/ASVspoof5.dev.track_1.tsv',is_train=False,is_eval=False)
    
    # print('no. of validation trials',len(file_dev))
    
    # dev_set = Dataset_ASVspoof2019_train(args,list_IDs = file_dev,labels = d_label_dev,base_dir = '/export/fs05/aurosweta/Big_Datasets/ASVspoof_2024/dev_set/',algo=args.algo)
    ...
    # val_loss = 0.0
    # val_acc = 0.0  # default to 0 in case accuracy fails

    # try:
    #     val_loss, val_acc = evaluate_accuracy(dev_loader, model, device)
    # except Exception as e:
    #     print(f"[Warning] Validation accuracy failed:{e}")
    #     # val_loss is still returned correctly
    #     val_acc = 0.0
