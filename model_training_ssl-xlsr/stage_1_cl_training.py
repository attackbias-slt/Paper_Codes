...
import yaml
from data_utils_SSL_19 import genSpoof_list,Dataset_ASVspoof2019_train,Dataset_ASVspoof2021_eval,Dataset_in_the_wild_eval
...
def evaluate_accuracy(dev_loader, model, device):
    ...
    for batch_x, batch_y in tqdm(dev_loader):
        ...
        # Count total and correct per class
    ...
    bon_acc = 100 * bonafide_correct / bonafide_total if bonafide_total > 0 else 0.0
    spoof_acc = 100 * spoof_correct / spoof_total if spoof_total > 0 else 0.0
...
if __name__ == '__main__':
    ...
    model_tag = 'stage1_model_check acc_{}_{}_{}_{}_{}'.format(
...
for epoch in range(num_epochs):
    ...
    val_loss, acc, bon_acc, spoof_acc = evaluate_accuracy(dev_loader, model, device)

    writer.add_scalar('loss', running_loss, epoch)
    writer.add_scalar('val_loss', val_loss, epoch)
    writer.add_scalar('val_acc', acc, epoch)
    writer.add_scalar('val_acc_bonafide', bon_acc, epoch)
    writer.add_scalar('val_acc_spoof', spoof_acc, epoch)

    print(f"\nEpoch {epoch} | Train Loss: {running_loss:.4f} | Val Loss: {val_loss:.4f} | Acc: {acc:.2f}% | Bonafide Acc: {bon_acc:.2f}% | Spoof Acc: {spoof_acc:.2f}%")

    torch.save(model.state_dict(), os.path.join(model_save_path, f'epoch_{epoch}.pth'))


    if patience_counter >= 3:
        print("Early stopping triggered, best model was from epoch:", epoch - 1)
        break
